(()=>{
 ...
 async function writeSnapshot(action='Saved data',record='System'){
  ...
  try{const payload=projectSnapshot(prj);payload.action=action;payload.record=record;await insert('form_instances',{project_id:prj.id,title:'R1 Runtime Snapshot',status:'complete',form_data:payload,completed_at:new Date().toISOString()});state.persistenceHealth=state.persistenceHealth||{};state.persistenceHealth.lastCloudWrite=new Date().toISOString();state.persistenceHealth.cloudError=null;return true}
 ...
 async function loadSnapshot(force=false){const prj=current();if(!prj?.id||!token())return false;if(!force&&lastLoaded===prj.id)return true;try{const rows=await rest('form_instances',`?project_id=eq.${encodeURIComponent(prj.id)}&title=eq.${encodeURIComponent('R1 Runtime Snapshot')}&select=form_data,updated_at,created_at&order=created_at.desc&limit=1`);const snap=rows?.[0]?.form_data;if(snap?.kind==='r1_project_snapshot'&&snap.state){mergeProjectSnapshot(prj,snap.state);lastLoaded=prj.id;state.persistenceHealth=state.persistenceHealth||{};state.persistenceHealth.lastCloudLoad=new Date().toISOString();state.persistenceHealth.cloudError=null;if(typeof renderAll==='function')renderAll();if(typeof refreshBanner==='function')refreshBanner();return true}}catch(e){state.persistenceHealth=state.persistenceHealth||{};state.persistenceHealth.cloudError=e.message;console.warn('TotalConstruct project cloud reopen failed',e)}return false}
 ...
 window.tcCloud={rest,insert,writeSnapshot,loadSnapshot,writeCompanySnapshot,loadCompanySnapshot,getCompanyId,projectTracked,companyTracked};
