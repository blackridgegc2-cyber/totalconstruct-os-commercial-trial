(()=>{
 ...
 const tracked=['meetings','daily','tasks','tm','quality','incidents','equipment','warranties','subcontractorProfiles','compliance','closeoutItems','warrantyRequests','bidders','savedReports','procurement','contracts','correspondence','costLines','purchases','fixedAssets','payroll401k','fieldReports','aeObservations','projectPhotos','designIssues','veItems','ownerDecisions','projectStatusUpdates'];
 ...
 function snapshot(prj){const data={};for(const k of tracked){if(state&&state[k]!==undefined)data[k]=state[k]}return {kind:'r1_runtime_snapshot',version:1,project_id:prj.id,project_name:prj.name,saved_at:new Date().toISOString(),state:data}}
 ...
 async function loadSnapshot(force=false){
  ...
  try{
   ...
   const snap=rows?.[0]?.form_data;if(snap?.kind==='r1_runtime_snapshot'&&snap.state){for(const [k,v] of Object.entries(snap.state))state[k]=v;lastLoaded=prj.id;state.persistenceHealth=state.persistenceHealth||{};state.persistenceHealth.lastCloudLoad=new Date().toISOString();state.persistenceHealth.cloudError=null;if(typeof renderAll==='function')renderAll();if(typeof refreshBanner==='function')refreshBanner();return true}
 ...
 window.tcCloud={rest,insert,writeSnapshot,loadSnapshot};
