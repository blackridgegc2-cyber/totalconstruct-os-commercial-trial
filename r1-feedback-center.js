(()=>{
  ...
  const canManage=()=>/executive|admin|operations|platform/.test(role());
  ...
  function ensureShell(){
    ...
    if(!document.getElementById('tcFeedbackButton')){const b=document.createElement('button');b.id='tcFeedbackButton';b.className='btn primary';b.textContent='Feedback / Report Problem';b.onclick=()=>{document.getElementById('tcFeedbackDrawer')?.classList.add('open');render()};document.body.appendChild(b)}
    if(!document.getElementById('tcFeedbackDrawer')){const d=document.createElement('div');d.id='tcFeedbackDrawer';d.innerHTML='<div id="tcFeedbackPanel"></div>';d.onclick=e=>{if(e.target===d)d.classList.remove('open')};document.body.appendChild(d)}
  }
  function render(){
    ensureShell();const p=document.getElementById('tcFeedbackPanel');if(!p)return;
    ...
    const q=document.getElementById('tcQuarterlyReview');if(q)q.onclick=()=>{const list=quarterlyReview();list.forEach(x=>update(x.id,{status:'AI Reviewing',aiAssessment:x.recommendation}));alert(`Quarterly review prepared ${list.length} backlog item${list.length===1?'':'s'} for management review.`)};
  }
  ...
  window.tcFeedbackCenter={submit,update,list:get,quarterlyReview,severity,render,open:()=>{ensureShell();document.getElementById('tcFeedbackDrawer').classList.add('open');render()}};
