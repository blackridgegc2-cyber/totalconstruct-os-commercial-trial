...
for(const token of ["'accounting'","'equipment'","'moduleConfig'","'storage'","'vaultProjects'","'legalHolds'","'invites'","'audit'","'taxModel'","'companyHealth'","'capitalPlan'","'overhead'"])if(!cloud.includes(token))errors.push(`company snapshot missing corporate collection: ${token}`);
...
for(const token of ["'taxModel'","'companyHealth'","'capitalPlan'","'overhead'"])if(!companyList.includes(token))errors.push(`Executive/CPA company assumption missing from company persistence: ${token}`);
